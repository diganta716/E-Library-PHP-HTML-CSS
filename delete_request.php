	...
	<div class="page">
			...
			<div class="body">
                ...
                <body>
					...
					<?php
					$uid = $_SESSION["UID"];
					$sql = "SELECT * FROM book WHERE PID='$uid'";
					...
					if (mysqli_num_rows($result) != 0) {
						...
						while($row = mysqli_fetch_array($result)) {
                            ...
                            <td>". $row[8]."</td>
                            <td>". $row[9]."</td>
                            <td><a href = 'pub_update.php?bookid=$row[0]&isbn=$row[1]&title=$row[2]&edition=$row[3]&desc=$row[4]
                            &totalpage=$row[5]&rating=$row[6]&pubdate=$row[7]&pid=$row[8]&pdf=$row[9]'>EDIT</a></td>
                            <td><a href = 'request_process.php?bookid=$row[0]'>DELETE</a></td>
